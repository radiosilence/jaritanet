...
export function createLocalServer(
  ...
  const containers = [
    {
      ...
      volumeMounts: Object.entries(persistence).map(([key, volume]) => ({
        name: `${key}-claim`,
  ...
  const volumeClaimTemplates = Object.entries(persistence).map(
    ([key, volume]) => ({
      metadata: {
        name: `${key}-claim`,
      },
  ...
  for (const [key, volume] of Object.entries(persistence)) {
    new k8s.core.v1.PersistentVolume(`${name}-${key}-vol`, {
      spec: {
        capacity: {
          storage: volume.storageSize,
        },
