...
- **Extracted, and consumed as published packages** — `@radiosilence/mcp-gateway-pulumi`, `@radiosilence/mariastew-pulumi` and `@radiosilence/slsk-mcp-pulumi` (the Soulseek client: shares and files into the music library on the file node, registered with the MCP gateway by URL rather than deployed by it). Each is published by the repository that builds the app, at that app's own version, so pinning the package says exactly which build runs and what image that is stops being this repository's business
