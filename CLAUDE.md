...
- **Hysteria2** — QUIC/UDP transport with Salamander obfuscation; the fast, loss-tolerant daily-driver entry, on the gateway and every edge. Listens on `:443` plus `altPorts` (3478 STUN, 4500 IPsec NAT-T) as separate `hysteria-server@` instances, because inspecting middleboxes block QUIC on 443 and VoIP-blocking regimes block 3478 — the client's urltest finds whichever survives. Auth is a per-user `userpass` map; obfs is server-wide. Guests get their own `guestPorts` listeners carrying a deny ACL (tailnet, loopback, RFC1918), because Hysteria2's ACL cannot match on the authenticated user — the fence has to be the process, not the credential. All minted on-box and delivered inside sing-box profiles.
